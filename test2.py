...
def load_image(file):
    """
    Load and auto-orient an image OR convert a PDF into a list of images.
    Returns:
        - Single PIL.Image object if normal image
        - List of PIL.Image objects if multi-page PDF
    """
    if not file:
        return None
    ...
    # Case 1: PDF file -> Convert all pages to images
    if filename.endswith(".pdf"):
        ...
        for page_num in range(len(doc)):
            # pix = doc[page_num].get_pixmap(dpi=200)  # good quality (Dots Per Inch = resolution)
            pix = doc[page_num].get_pixmap(dpi=100) 

            img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
            images.append(ImageOps.exif_transpose(img))
        return images

    # Case 2: Normal image -> Just load & auto-orient
...
#this to convert the image to pdf also saport multiple image to convert pdf
def convert_images_to_pdf(files):
    """Convert one or multiple images (or PDF pages) into a single PDF, keeping aspect ratio per page."""
    pdf_buffer = BytesIO()
    ...
    for file in files:
        try:
            ...
            # If it's a list (multi-page PDF converted to images)
...
def generate_document(first_image, back_image, first_image_2, back_image_2,
    ...
    if layout == "ONENOTARY":
        ...
        if front_image:
            ...
            # c.drawImage(ImageReader(front_image),  (page_width-230)/2, 370, width=230, height=190)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if multiPagePdf_files:
            if len(multiPagePdf_files) == 1 and multiPagePdf_files[0].name.lower().endswith(".pdf"):
                #  Single PDF uploaded -> keep as is
                multiPagePdf = multiPagePdf_files[0]
            else:
                # Convert images (or multiple PDFs-as-images) -> one PDF
                converted_pdf = convert_images_to_pdf(multiPagePdf_files)
                if converted_pdf:
                    multiPagePdf = converted_pdf
